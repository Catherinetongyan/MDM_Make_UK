...
def predict_future(target_col, df, future_years):
    ...
    for year in future_years:
        input_data = pd.DataFrame({'Year': [year], 'Crisis_Dummy': [0]})
...
results_df.to_csv('forecast_results_2030.csv', index=False)
print("Data saved to forecast_results_2030.csv")
...
plt.title("Projected Age Structure (1993-2035)")
plt.xlabel("Year")
plt.ylabel("% of Workforce")
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('age_trend_graph.png')
print("Graph saved as age_trend_graph.png")
...
plt.axvline(x=2024, color='gray', linestyle='--', label='Forecast Start')
plt.title("Projected Ethnicity Trends (2004-2035)")
plt.xlabel("Year")
plt.ylabel("% of Workforce")
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('ethnicity_trend_graph.png')
print("Graph saved as ethnicity_trend_graph.png")